...
const SECTORS: Sector[] = sectorsConfig.map(sector => ({
  ...
  color: sector.id === "municipal" ? "blue" : 
         sector.id === "commercial" ? "purple" :
         sector.id === "industrial" ? "gray" :
         sector.id === "residential" ? "green" : "gray",
...
export function PremiumProjectWizard({ open, onOpenChange, onProjectCreated }: PremiumProjectWizardProps) {
  ...
  const canContinue = useMemo(() => {
    switch (currentStep) {
      case 1:
        return projectData.name.trim() !== "" && projectData.client.trim() !== ""
      case 2:
  ...
  const handleCreateProject = useCallback(async () => {
    ...
    try {
      const newProject = await createProject({
        ...
        description: projectData.description || `Proyecto ${projectData.sector.toLowerCase()} para ${projectData.client}`,
      ...
      try {
        ...
        if (template) {
          ...
          console.log(`✅ Applied template "${template.name}" to project ${newProject.id}`)
        }
      } catch (templateError) {
        console.warn('⚠️ Failed to apply template, will use fallback on load:', templateError)
      }
      */
      ...
      router.push(routes.project.technical(newProject.id, { quickstart: true }))
  ...
  }, [canContinue, projectData, createProject, onProjectCreated, onOpenChange, router])
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => updateProjectData({ client: e.target.value })}
                  className="h-12 text-base"
                />
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-12 gap-6">
              ...
              <div className="md:col-span-5 space-y-3">
                ...
                <RadioGroup
                  ...
                  onValueChange={(value) => updateProjectData({
                    sector: value,
                    ...
                    <ScrollArea className="h-[320px] rounded-lg border border-border">
                      <div className="p-3">
                        <RadioGroup
                          ...
                          onValueChange={(value) => updateProjectData({ subsector: value })}
                          className="gap-1"
                        >
                          {selectedSector.subsectors.map((subsectorId) => {
                            const subsectorLabel = getSubsectorLabel(projectData.sector, subsectorId)
                            const isSelected = projectData.subsector === subsectorId
                    ...
                    <div className="rounded-lg bg-muted/50 p-3 border border-border">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          <span className="font-medium text-foreground">Tip:</span> Select the most specific subsector that matches your project for better recommendations
                        </div>
                      </div>
                    </div>
                  </>
        ...
        return (
          <div className="space-y-6">
            <div className="text-center space-y-2">
              ...
              <p className="text-muted-foreground">
                Define where the project will be developed for technical considerations
              </p>
            </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedSector.examples.map((example) => (
                      <Badge
                        ...
                        onClick={() => updateProjectData({ location: `${example}, México` })}
                      >
              ...
              <CardContent className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-foreground">
                      {getSubsectorLabel(projectData.sector, projectData.subsector)}
                    </p>
                  </div>
