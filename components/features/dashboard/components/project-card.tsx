...
import type { ProjectStatus, ProjectSector } from "@/lib/project-types"
...
const ProjectCard = memo(function ProjectCard({
  ...
  const statusDescription = useMemo(() => getProjectStatusDescription(status), [status])
  const SectorIcon = useMemo(() => sectorIcons[sector] || Building, [sector])
  const formattedCreatedDate = useMemo(() => new Date(createdAt).toLocaleDateString("en-US"), [createdAt])
  const formattedUpdatedAt = useMemo(() => new Date(updatedAt).toLocaleDateString("en-US"), [updatedAt])
  ...
  const primaryAction: PrimaryAction = useMemo(() => {
    switch (status) {
      ...
      case "Completed":
        return { label: "View Documentation", href: `/project/${id}`, variant: "outline" }
      case "On Hold":
      default:
  ...
  return (
    <Card className={cn("group relative flex h-full flex-col bg-card/80 backdrop-blur-sm border border-border/50 shadow-lg hover:shadow-xl hover:-translate-y-1 hover:scale-[1.02] transition-all duration-300 overflow-hidden", className)}>
      ...
      <CardContent className="relative flex flex-1 flex-col gap-5 py-5 bg-gradient-to-b from-transparent to-card/20">
        ...
        <div className="mt-auto flex flex-col gap-2">
          <Link
            ...
            className={cn(
              buttonVariants({ size: "sm", variant: primaryAction.variant ?? "default" }),
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You are about to delete <strong>{name}</strong>. This action cannot be undone and will delete:
              <ul className="mt-2 list-disc list-inside space-y-1">
                <li>All captured technical information</li>
                <li>{proposalsCount} generated proposal{proposalsCount !== 1 ? 's' : ''}</li>
