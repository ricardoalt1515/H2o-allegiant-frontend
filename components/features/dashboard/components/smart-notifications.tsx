...
export function SmartNotifications() {
  ...
  const notifications = useMemo(() => {
    ...
    const readyForProposal = projects.filter(p =>
      p.status === "In Preparation" && p.progress >= 70
    )
    ...
    const incompleteProjects = projects.filter(p =>
      p.status === "In Preparation" && p.progress < 50
    )
                ...
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    <div className="mt-0.5">
                      <Icon className={cn("h-5 w-5", getIconColor(notification))} />
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {notification.project && (
                        <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
                          ...
                          <span>
                            Updated {new Date(notification.project.updatedAt).toLocaleDateString("en-US")}
                          </span>
                        </div>
                      )}
