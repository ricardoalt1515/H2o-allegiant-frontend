...
export function ProjectOverview({ project, onNavigateToTechnical }: ProjectOverviewProps) {
  const budgetLabel =
    typeof project.budget === "number" ? formatCurrency(project.budget) : "Not defined"
  const durationLabel = project.timeline ?? project.scheduleSummary ?? "Not defined"
  const updatedAtLabel = project.updatedAt
    ? new Date(project.updatedAt).toLocaleDateString("en-US")
    : "Not defined"
  const teamMembers = project.team && project.team.length > 0 ? project.team : DEFAULT_TEAM
        ...
        <Card>
          <CardHeader className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            ...
            {onNavigateToTechnical && (
              <Button size="sm" variant="outline" onClick={onNavigateToTechnical}>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-secondary">{project.status}</div>
                <div className="text-xs text-muted-foreground">Current Status</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-muted-foreground">{durationLabel}</div>
                <div className="text-xs text-muted-foreground">Estimated Duration</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-chart-1">{budgetLabel}</div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">{updatedAtLabel}</div>
              </div>
            </div>
          </CardContent>
        </Card>
