...
import { PROJECT_STATUS_FLOW, getProjectStatusStep } from "@/lib/project-status"
...
export function ProjectProgressIndicator({ status, className }: ProjectProgressIndicatorProps) {
  const currentStep = Math.min(getProjectStatusStep(status), PROGRESS_STEPS.length)
