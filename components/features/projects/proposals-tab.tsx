...
import { FileText, Download, Eye, Sparkles, ClipboardCheck, Lightbulb, Trash2 } from "lucide-react"
import { IntelligentProposalGeneratorComponent } from "./intelligent-proposal-generator"
import { EmptyState } from "@/components/ui/empty-state"
import { useTechnicalSections, useCurrentProject, useTechnicalDataActions, useLoadProjectAction } from "@/lib/stores"
import { overallCompletion, sectionCompletion } from "@/lib/technical-sheet-data"
import type { ProjectDetail, ProposalVersion } from "@/lib/project-types"
import { formatCurrency } from "@/lib/utils"
import { routes, ProjectTab, ProposalView } from "@/lib/routes"
...
export function ProposalsTab({ project }: ProposalsTabProps) {
  ...
  const PROPOSAL_STATUS_LABELS: Record<ProposalVersion["status"], string> = {
    Draft: "Draft",
    Current: "Current",
    Archived: "Archived",
  }

  const PROPOSAL_TYPE_LABELS: Record<ProposalVersion["type"], string> = {
        ...
        <Card className="alert-warning-card">
          <CardHeader className="space-y-2">
            ...
            <CardDescription className="opacity-90">
              Complete the technical sheet so the conceptual agent has sufficient data.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <Alert variant="default" className="border-warning/30 bg-card/50">
              ...
              <AlertDescription>
                <span className="font-semibold">Current progress: {completion.percentage}% complete.</span> At least {readinessThreshold}% required.
              </AlertDescription>
            </Alert>
            {prioritizedGaps.length > 0 && (
              <div className="space-y-4">
                <div className="space-y-2 text-sm">
                  <p className="font-semibold">Completa estas secciones para continuar:</p>
                  <div className="space-y-2">
                    {prioritizedGaps.map(({ section, stats }) => (
                      <div key={section.id} className="flex items-center justify-between p-3 bg-card/60 rounded-lg border border-border/60 backdrop-blur">
                        <div>
                          <p className="font-medium">{section.title}</p>
                          <p className="text-xs text-muted-foreground">{stats.completed} de {stats.total} campos</p>
                        </div>
        ...
        {proposals.length === 0 ? (
          <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">No Proposals Generated</h3>
              <p className="text-muted-foreground max-w-md mb-6">
                Once you complete the technical sheet to {readinessThreshold}%, you can generate automated conceptual proposals with AI.
              </p>
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                >
                  {isReady ? "Generate First Proposal" : "Complete Technical Sheet"}
                </Button>
                <Button
                  variant="outline"
                  onClick={() => router.push(routes.project.overview(project.id))}
                >
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <Badge variant="outline">{PROPOSAL_TYPE_LABELS[proposal.type]}</Badge>
                      </div>
                    </div>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="text-lg font-semibold">{formatCurrency(proposal.capex)}</div>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-muted-foreground">OPEX Anual</label>
                      <div className="text-lg font-semibold">{formatCurrency(proposal.opex)}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={() => router.push(routes.project.proposal.detail(project.id, proposal.id))}
                    >
                    ...
                    <Button
                      ...
                      className="flex-1 bg-transparent"
                      onClick={() =>
                        router.push(routes.project.proposal.pdf(project.id, proposal.id))
                      }
                    >
                    ...
                    <Button
                      ...
                      onClick={() => openDeleteDialog(proposal.id, proposal.title)}
