...
import { AlertCircle, Brain, CheckCircle2, Info, Lightbulb, XCircle } from 'lucide-react'
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs'
import { Badge } from '@/components/ui/badge'
import { Alert, AlertDescription } from '@/components/ui/alert'
import { Skeleton } from '@/components/ui/skeleton'
import { ProposalsAPI, type AIMetadata } from '@/lib/api/proposals'
 ...
 */
export function AITransparency({ projectId, proposalId, className }: AITransparencyProps) {
  ...
  useEffect(() => {
    ...
    async function loadMetadata() {
      try {
        ...
        if (mounted) {
          setError(err instanceof Error ? err.message : 'Failed to load AI metadata')
        }
      } finally {
        if (mounted) {
          setIsLoading(false)
      ...
      <CardContent className="space-y-4">
        <Tabs defaultValue="proven-cases" className="w-full">
          ...
          <TabsContent value="proven-cases" className="space-y-4 mt-4">
            <div className="text-sm text-muted-foreground">
              The AI consulted <strong>{metadata.proven_cases.length}</strong> similar proven cases
              {metadata.user_sector && ` from the ${metadata.user_sector} sector`}
            </div>
            ...
            {metadata.proven_cases.length === 0 ? (
              <Alert>
                <Info className="h-4 w-4" />
                <AlertDescription>
                  No proven cases were consulted. The AI designed this solution from first principles.
                </AlertDescription>
              </Alert>
            ) : (
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {provenCase.similarity_score !== undefined && (
                        <div className="flex items-center gap-1.5">
                          ...
                          <Badge 
                            variant={provenCase.similarity_score >= 0.9 ? 'default' : 'secondary'}
                            className={provenCase.similarity_score >= 0.9 ? 'bg-success text-success-foreground' : ''}
                          >
          ...
          <TabsContent value="assumptions" className="space-y-4 mt-4">
            ...
            <Alert className="bg-blue-50/50 dark:bg-blue-950/20 border-blue-200/50 dark:border-blue-800/50">
              ...
              <AlertDescription className="text-blue-700 dark:text-blue-300">
                <strong>Note:</strong> Verify these assumptions align with your project's specific conditions
                ...
                {metadata.alternatives.map((alternative, index) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">{alternative.technology}</span>
                    </div>
                    <div className="text-sm text-muted-foreground pl-6">
                      <strong>Reason not selected:</strong> {alternative.reason_rejected}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <TabsContent value="justification" className="space-y-4 mt-4">
            ...
            {metadata.technology_justification.length === 0 ? (
              ...
              <div className="space-y-3">
                {metadata.technology_justification.map((justification, index) => (
                  <div
                    key={index}
                    className="rounded-lg border bg-card p-4 space-y-3 hover:bg-accent/50 transition-colors"
                  >
        ...
        <div className="pt-4 border-t">
          <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg">
            <div className="flex items-center gap-2">
              {metadata.confidence_level === 'High' && <CheckCircle2 className="h-5 w-5 text-success" />}
              {metadata.confidence_level === 'Medium' && <AlertCircle className="h-5 w-5 text-amber-500" />}
              {metadata.confidence_level === 'Low' && <XCircle className="h-5 w-5 text-destructive" />}
              <span className="font-medium">AI Confidence Level:</span>
            </div>
            <Badge
              variant={
                metadata.confidence_level === 'High' ? 'default' :
                metadata.confidence_level === 'Medium' ? 'secondary' :
                'destructive'
              }
              className={
          ...
          <div className="space-y-2 p-3 bg-blue-50/50 dark:bg-blue-950/20 rounded-lg border border-blue-200/50 dark:border-blue-800/50">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{metadata.usage_stats.model_used}</span>
              </div>
              <div className="flex justify-between">
