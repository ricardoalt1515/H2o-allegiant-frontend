...
// Simple helper functions (moved from proposal-data-helpers)
function getDesignFlowRate(proposal: Proposal): number | null {
	return proposal.aiMetadata?.technicalData?.flowRateM3Day || null;
}
...
function calculateEquipmentUtilization(
	...
	if (!designFlowM3Day || !equipment.capacityM3Day) return null;
...
export function EquipmentListImproved({
	equipmentList,
	proposal,
}: EquipmentListImprovedProps) {
	...
	const criticalEquipment = sortedEquipment.filter(
		(eq) => eq.criticality === "high",
	);
	const nonCriticalEquipment = sortedEquipment.filter(
		(eq) => eq.criticality !== "high",
	);
			...
			<Card className="aqua-panel">
				<CardHeader>
					...
					<div className="grid grid-cols-3 gap-4 pt-4 text-center text-sm">
						...
						<div>
							<div className="text-2xl font-bold">
								{equipmentList
									.reduce((sum, eq) => sum + eq.powerConsumptionKw, 0)
									.toFixed(0)}
							</div>
				...
				<CardContent className="space-y-3">
					{visibleEquipment.map((equipment, idx) => {
						...
						const _utilization = calculateEquipmentUtilization(
							equipment,
							designFlowM3Day,
						);
						...
						return (
							<Collapsible
								key={`${equipment.type}-${idx}`}
								defaultOpen={equipment.criticality === "high"}
							>
								<Card className={cn("overflow-hidden", config.borderColor)}>
									...
									<CollapsibleContent>
										<CardContent className="space-y-4 pt-0">
											{/* Capacity Progress - Simple inline replacement */}
											{hasDesignFlow &&
												designFlowM3Day &&
												equipment.capacityM3Day > 0 && (
											...
											{!hasDesignFlow && (
												<Alert variant="default" className="border-muted">
													<AlertDescription className="text-xs text-muted-foreground">
														Utilization metrics unavailable without design flow
														rate from agent.
													</AlertDescription>
												</Alert>
											)}
