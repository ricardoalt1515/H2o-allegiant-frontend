...
import { SOURCE_CONFIG, IMPORTANCE_CONFIG } from "@/lib/constants"
import type { TableField, TableSection, DataSource } from "@/lib/types/technical-data"
...
interface EngineeringDataTableProps {
  ...
  onBulkEdit?: (changes: Array<{ sectionId: string; fieldId: string; value: any; unit?: string }>) => void
  className?: string
}
...
const importanceConfig = {
  critical: { label: IMPORTANCE_CONFIG.critical.label, color: IMPORTANCE_CONFIG.critical.color },
  recommended: { label: IMPORTANCE_CONFIG.recommended.label, color: IMPORTANCE_CONFIG.recommended.color },
  optional: { label: IMPORTANCE_CONFIG.optional.label, color: IMPORTANCE_CONFIG.optional.color }
}
...
export function EngineeringDataTable({
  ...
  const columns: ColumnDef<FieldRow>[] = [
        ...
        return (
          <div className="flex items-center gap-2">
            ...
            {field.importance && (
              <Badge
                variant="outline"
                className={cn("text-xs", importanceConfig[field.importance].color)}
              >
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const handleSave = (sectionId: string, fieldId: string, value: string | number | string[], unit?: string, _notes?: string) => {
  ...
  const completedFields = data.filter(field => field.value || field.value === 0).length
    ...
    <TooltipProvider>
      <div className={cn("space-y-4", className)}>
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-center">
                  <div className="text-xl font-semibold">{completedFields}/{totalFields}</div>
              ...
              {Object.entries(sourceConfig).map(([key, config]) => (
                <DropdownMenuCheckboxItem
                  ...
                  onCheckedChange={(checked) => {
                    table.getColumn("source")?.setFilterValue(checked ? undefined : key)
                  }}
                >
                ...
                <TableRow>
                  ...
                  >
                    <div className="flex flex-col items-center gap-2">
                      <div className="text-muted-foreground">No parameters to display</div>
        ...
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          <div>
            Showing {table.getRowModel().rows.length} of {data.length} parameters
          </div>
          <div className="flex gap-4">
            <span>✅ {data.filter(f => f.source === "manual").length} Manual</span>
            <span>📁 {data.filter(f => f.source === "imported").length} Imported</span>
