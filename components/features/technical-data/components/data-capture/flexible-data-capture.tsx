...
import { Plus, Library, Settings, Save } from "lucide-react"
...
export function FlexibleDataCapture({
  ...
  const [accordionValue, setAccordionValue] = useState<string[]>(() => {
    const incompleteSections = sections
      ...
      .filter(s => {
        const completed = s.fields.filter(f => f.value && f.value !== "").length
  ...
  const handleAddParametersFromLibrary = useCallback((parameters: TableField[]) => {
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              {accordionValue.length + fixedAccordionValue.length} of {sections.length} sections expanded
            </p>
            <div className="flex gap-2">
              <Button
                ...
                onClick={() => {
                  ...
                  const fixedSections = sections.filter(s => isFixedSection(s.id))
              ...
              <Button
                ...
                disabled={accordionValue.length === 0 && fixedAccordionValue.length === 0}
              >
                Collapse All
              </Button>
            </div>
          ...
          <Accordion
            ...
            className="space-y-4"
          >
            {sections.filter(s => !isFixedSection(s.id)).map((section) => (
