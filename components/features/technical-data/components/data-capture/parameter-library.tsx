...
import { Search, Plus, Beaker, Droplet, Leaf, Bug, Settings, Scale } from "lucide-react"
...
const CATEGORY_CONFIG: Record<ParameterCategory, { label: string; icon: any; color: string }> = {
  ...
  "chemical-inorganic": { label: "Chemical Inorganic", icon: Beaker, color: "bg-purple-500" },
  "chemical-organic": { label: "Chemical Organic", icon: Leaf, color: "bg-green-500" },
...
export function ParameterLibrary({
  ...
  const [selectedCategory, setSelectedCategory] = useState<ParameterCategory | null>(null)
  ...
  const availableParams = useMemo(() => {
    ...
    return params
  }, [sectionId, sector, subsector, existingFieldIds, selectedCategory, searchTerm])
