...
import { Zap, Calculator, FileText, Play, CheckCircle2 } from "lucide-react"
  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-base focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        ...
        calculate: "bg-blue-600 text-white hover:bg-blue-700 shadow-sm hover:shadow-md transition-all",
        generate: "bg-gradient-to-r from-purple-600 to-blue-600 text-white hover:from-purple-700 hover:to-blue-700 shadow-sm hover:shadow-md",
        technical: "bg-slate-800 text-slate-100 hover:bg-slate-700 border border-slate-600",
        success: "bg-green-600 text-white hover:bg-green-700",
        critical: "bg-orange-600 text-white hover:bg-orange-700 animate-pulse",
        quick: "bg-gradient-to-r from-emerald-500 to-teal-600 text-white hover:from-emerald-600 hover:to-teal-700 shadow-lg hover:shadow-xl transform hover:scale-105 transition-all"
      },
      size: {
...
const EngineeringButton = React.forwardRef<HTMLButtonElement, EngineeringButtonProps>(
  ({
    className,
    variant,
    size,
    ...
    return (
      <Comp
        className={cn(engineeringButtonVariants({ variant, size, loading, className }))}
        ref={ref}
        disabled={isDisabled}
        {...props}
      >
...
interface QuickActionButtonProps extends Omit<EngineeringButtonProps, "variant"> {
  action: "calculate-capex" | "generate-proposal" | "validate-parameters" | "export-pdf" | "quick-start"
}

export function QuickActionButton({ action, ...props }: QuickActionButtonProps) {
  const actionConfig = {
    "calculate-capex": {
