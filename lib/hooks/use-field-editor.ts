...
type FieldEditorAction =
  ...
  | { type: 'CANCEL'; payload: { value: string | number | string[]; unit: string; notes: string } }
  | { type: 'SAVE' }
  | { type: 'SAVE_NOTES' }

function fieldEditorReducer(state: FieldEditorState, action: FieldEditorAction): FieldEditorState {
  switch (action.type) {
    case 'START_EDIT':
    ...
    case 'START_DELETE':
      return { ...state, mode: state.mode === 'deleting' ? 'viewing' : 'deleting' }
    case 'UPDATE_VALUE':
...
interface UseFieldEditorOptions {
  field: TableField
  onSave?: (value: string | number | string[], unit?: string, notes?: string) => void
  autoSave?: boolean
