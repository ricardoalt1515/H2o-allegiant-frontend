export type Sector = "commercial" | "residential" | "industrial" | "municipal" | "other"
...
export const sectorsConfig: SectorConfig[] = [
  {
    id: "commercial",
    label: "Commercial",
    description: "Projects for restaurants, hotels, shopping malls, and food service",
    subsectors: [
...
export const getSubsectors = (sectorId: Sector): { id: Subsector; label: string }[] => {
  const sector = getSectorConfig(sectorId)
  return sector?.subsectors || []
}

export const getSectorBySubsector = (subsectorId: Subsector): SectorConfig | undefined => {
...
export const sectorApplications = {
  "commercial": {
    "restaurant": {
      commonParameters: ["turbidity", "ph", "chlorine", "hardness", "bacterial-quality", "heavy-metals"],
      priorities: ["product-quality", "regulatory-compliance", "process-efficiency"],
      description: "Food quality and strict regulatory compliance"
    },
    "hotel": {
      commonParameters: ["hardness", "chlorine-residual", "iron", "ph", "temperature"],
    ...
    "office_building": {
      commonParameters: ["turbidity", "ph", "chlorine-residual", "bacterial-quality"],
      priorities: ["employee-health", "cost-efficiency", "maintenance"],
      description: "Safe and economical water for employees"
    },
    "food_service": {
      commonParameters: ["turbidity", "ph", "chlorine", "hardness", "bacterial-quality"],
  ...
  "residential": {
    "single_home": {
      commonParameters: ["hardness", "iron", "ph", "nitrates", "bacterial-quality"],
      priorities: ["family-health", "appliance-protection", "cost"],
      description: "Safe water and appliance protection for families"
    },
    "multi_family": {
      commonParameters: ["turbidity", "chlorine-residual", "ph", "hardness", "flow-rate"],
      priorities: ["resident-satisfaction", "system-reliability", "operational-cost"],
  ...
  "industrial": {
    ...
    "textile_manufacturing": {
      commonParameters: ["hardness", "iron", "ph", "turbidity", "color", "temperature"],
      priorities: ["process-quality", "equipment-protection", "wastewater-treatment"],
      description: "Process water and textile effluent treatment"
    },
    "pharmaceutical_manufacturing": {
      commonParameters: ["conductivity", "ph", "bacterial-quality", "heavy-metals", "organics"],
      priorities: ["regulatory-compliance", "product-purity", "validation"],
      description: "Ultrapure water and pharmaceutical compliance"
    },
    "chemical_processing": {
      commonParameters: ["ph", "conductivity", "heavy-metals", "organics", "temperature"],
  ...
  "municipal": {
    "government_building": {
      commonParameters: ["turbidity", "ph", "chlorine-residual", "fluoride", "bacterial-quality"],
      priorities: ["public-health", "regulatory-compliance", "cost-efficiency"],
      description: "Safe drinking water for public services"
    },
    "water_utility": {
      commonParameters: ["turbidity", "ph", "chlorine", "fluoride", "bacterial-quality", "capacity"],
      priorities: ["population-coverage", "regulatory-compliance", "sustainability"],
