...
function mergeSectionConfigs(
	chain: TemplateConfig[],
): Map<string, SectionConfig> {
	...
	for (const template of chain) {
		for (const section of template.sections) {
			...
			const mergedSection: SectionConfig = {
				...existing,
				...section,
				addFields: finalFields,
			};

			// Only add fieldOverrides if non-empty (DRY: avoid undefined noise)
			if (Object.keys(fieldOverrides).length > 0) {
				mergedSection.fieldOverrides = fieldOverrides;
			}

			mergedSections.set(section.id, mergedSection);
		}
	}
	...
	for (const [sectionId, config] of sectionConfigs) {
		...
		for (const fieldId of config.addFields ?? []) {
			...
			if (field) {
				...
				console.warn(
					`⚠️ Field "${fieldId}" in section "${sectionId}" not found in parameter library`,
				);
			}
		}
...
export function applyTemplate(
	...
	console.log(
		`✅ Template applied: ${totalFields} fields across ${sections.length} sections`,
	);
...
export function createTemplateRegistry(): TemplateRegistry {
	...
	try {
		const {
			INDUSTRIAL_TEMPLATE,
		} = require("./sector-templates/industrial.template");
	...
	// Subsector templates
	// Food processing template not restored yet (future work)

	try {
		const {
			OIL_GAS_TEMPLATE,
		} = require("./subsector-templates/oil-gas.template");
